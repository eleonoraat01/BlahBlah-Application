  ...
  <Dialog v-if="channelStore.selectedChannel" :open="dialogState" @update:open="dialogState = $event">
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Are you sure you want to delete "{{ channelStore.selectedChannel.name }}" from your channels?
          <br>
          This action will remove the channel and all its content for all members.
        </DialogDescription>
      </DialogHeader>
...
async function handleSubmit() {
  ...
  try {
    await channelStore.deleteSelectedChannel();

    toast({ description: `You have successfully deleted the "${channelName}" channel` });
    resetState();
  } catch (error) {
