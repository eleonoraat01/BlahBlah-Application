  ...
  <Dialog :open="dialogState" @update:open="dialogState = $event">
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Enter your friend's ID to send them a request
        </DialogDescription>
      </DialogHeader>

      <Label class="sr-only" for="addFriend">Enter friend's UserID</Label>
      <Input id="addFriend" v-model="inputValue" type="text" placeholder="Enter friend's UserID" :disabled="loadingStore.isLoading" @keydown.enter.prevent="handleSubmit" />
...
async function handleSubmit() {
  ...
  if (!inputValue.value || isNaN(Number(inputValue.value))) {
    errorMessage.value = 'Please enter a valid friend\'s UserID';
    return;
  }

  try {
    await userStore.addFriend(Number(inputValue.value));
    resetState();
  } catch (error) {
